...
import StudentBlock from "../../miniComponents/studentsBlock"
...
const StudentData = ({data,loading,error, updateUI, onData, handleBack}: Props) =>{
    const [deleteConfirmation, setDeleteConfirmation] = useState<boolean>(false)
    const [value, setValue] = useState<string>("")
    
    // send edit id to the Student ui page
    const editStudent = (val:string) => {
     onData(val)
    }
  
    //set delete authentication to true
    const deleteStudent = (val:string) => {
      setValue(val)
    ...
    const deletePost = async (value: string) => {
      try {
        const response = await axios.delete(`${API_BASE_URL}/student` + value)
        updateUI()
      ...
      (
        ...
        data.slice().reverse().map((datas) => 
         <StudentBlock 
            ...
            onEditClick={() => editStudent(datas._id)}
            onDeleteClick={() => deleteStudent(datas._id)}
        />
        )
        }
        </div>
